...
import { IPC_CHANNELS, type ManagerCommand, type RendererBridge } from '../shared/ipc';
...
const bridge: RendererBridge = {
  ...
  subscribeCommands(callback) {
    const handler = (_event: unknown, command: ManagerCommand) => {
      callback(command);
    };

    ipcRenderer.on(IPC_CHANNELS.command, handler);
    return () => {
      ipcRenderer.off(IPC_CHANNELS.command, handler);
    };
  },
  ...
  moveTab: (runtimeId, toIndex) => ipcRenderer.invoke(IPC_CHANNELS.moveTab, runtimeId, toIndex),
